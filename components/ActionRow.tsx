...
export function ActionRow({ id, title, children, command, commandLabel, status, danger = false }: ActionRowProps) {
  return (
    <div className="grid grid-cols-[24px_1fr] gap-4 border-t border-zinc-800 py-6 first:border-t-0 first:pt-0">
      <ChecklistToggle id={id} label={title} />
      <div className="min-w-0">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <h3 className="text-[15px] font-semibold text-zinc-100">{title}</h3>
          {status && <span className={cn("font-mono text-[10px] uppercase tracking-wider", danger ? "text-red-400" : "text-zinc-600")}>{status}</span>}
        </div>
