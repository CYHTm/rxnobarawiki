  ...
  return (
    <div className="border-y border-zinc-800 py-6">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          <h3 className="text-[15px] font-semibold text-zinc-100">Прикидка андервольта</h3>
          <p className="mt-1 text-sm text-zinc-500">Начни с 1125 мВ. Ниже пойдешь только после нормального теста.</p>
        </div>
        <div className="text-left sm:text-right">
          <div className="font-mono text-3xl font-semibold text-white">{voltage} <span className="text-sm text-zinc-600">мВ</span></div>
          <div className="mt-1 text-xs text-zinc-500">{estimate.risk}</div>
        </div>
      </div>
      ...
      <div className="mt-6 grid grid-cols-3 border border-zinc-800">
...
function Metric({ value, label }: { value: string; label: string }) {
  return (
    <div className="border-r border-zinc-800 p-3 last:border-r-0 sm:p-4">
      <div className="font-mono text-sm font-semibold text-zinc-200 sm:text-base">{value}</div>
