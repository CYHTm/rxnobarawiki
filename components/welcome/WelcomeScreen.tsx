  ...
  return (
    <main className="welcome-screen">
      ...
      <motion.div
        ...
        animate={{ x: [0, 28, -12, 0], y: [0, -18, 16, 0], scale: [1, 1.08, 0.96, 1] }}
        transition={{ duration: 16, repeat: Infinity, ease: "easeInOut" }}
        aria-hidden="true"
      />
      <motion.div
        className="welcome-color-field welcome-color-field-two"
        animate={{ x: [0, -24, 15, 0], y: [0, 24, -12, 0], scale: [1, 0.94, 1.06, 1] }}
      ...
      <motion.div
        ...
        animate={leaving ? { opacity: 0, scale: 0.94, filter: "blur(16px)" } : { opacity: 1, scale: 1, filter: "blur(0px)" }}
      ...
      >
        ...
        <div className="welcome-layout">
          <section className="welcome-copy" aria-labelledby="welcome-title">
            <motion.p initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08, duration: 0.5 }} className="welcome-eyebrow">
              Добро пожаловать, кент
            </motion.p>
            <motion.h1 id="welcome-title" initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.14, type: "spring", stiffness: 160, damping: 22 }}>
              <span className="welcome-title-small">Настрой</span>
              <span className="welcome-title-shimmer">NOBARA</span>
            </motion.h1>
            <motion.p initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.22, duration: 0.55 }} className="welcome-lead">
            ...
            <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.5 }} className="welcome-actions">
              <MotionLink
                href="/guide"
                onClick={(event) => {
          ...
          >
            ...
            <motion.div
              ...
              animate={{ borderRadius: ["38% 62% 54% 46% / 45% 42% 58% 55%", "55% 45% 36% 64% / 52% 63% 37% 48%", "38% 62% 54% 46% / 45% 42% 58% 55%"] }}
            ...
            {floatingCards.map((card, index) => {
              ...
              return (
                <motion.div
                  ...
                  animate={{ y: [0, index % 2 === 0 ? -10 : 9, 0], rotate: [0, index % 2 === 0 ? 1.4 : -1.4, 0] }}
                  transition={{ duration: 5.5 + index, repeat: Infinity, ease: "easeInOut", delay: index * -0.7 }}
                >
